...
function ExpensesDetail() {
    ...
    const totalSpending = useMemo(() => {
        const rows = dataListDetail?.data || [];
        return rows.reduce((sum, r) => sum + Number(r.ExpensesAmount || 0), 0);
    }, [dataListDetail]);

    const totalBalance = useMemo(() => {
        if (!cardInfo) return 0;
        return Number(cardInfo.ExpensesBalance ?? cardInfo.ExpensesMoney ?? 0);
    }, [cardInfo]);
    ...
    const handleSaveAdd = async () => {
        const endpoint = GetApiEndPoint();
        await axios.put(endpoint + '/Expenses/CreateExpensesDetail', addData)
            .then(() => {
    ...
    return (
        <main className="flex min-h-screen flex-col items-center justify-between p-24">
            <div className="z-10 max-w-5xl w-full items-center justify-between text-sm lg:flex">
                ...
                <div className="fixed bottom-0 left-0 flex h-48 w-full items-end justify-center gap-2 bg-gradient-to-t from-white via-white dark:from-black dark:via-black lg:static lg:h-auto lg:w-auto lg:bg-none">
                    <button className="btn transition-colors duration-500 transform w-48 rounded-full" onClick={handleOpenAdd}>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
                        </svg>
                        Add Expense
                    </button>
                    <button className="btn transition-colors duration-500 transform w-48 rounded-full" onClick={() => { router.push('/') }}>
            ...
            <dialog ref={addModalRef} className="modal modal-bottom sm:modal-middle">
                <div className="modal-box">
                    ...
                    <div className="w-full flex justify-between py-4 space-x-4">
                        <select
                            className="select select-bordered w-full max-w-xs"
                            value={addData.ExpensesType}
                            onChange={(e) => setAddData({ ...addData, ExpensesType: e.target.value })}
                        >
                            <option value='FIXCOST'>Fix cost</option>
                            <option value='CREDIT'>Credit</option>
                            <option value='OTHER'>Other</option>
                        ...
                        <input type="text" placeholder="Description" className="input input-bordered w-full max-w-xs"
                            value={addData.ExpensesDesc || ''}
                            onChange={(e) => setAddData({ ...addData, ExpensesDesc: e.target.value })}
                        />
                    </div>
                    <div className="w-full flex flex-wrap gap-2 pb-3">
                        {descPresets.map((item) => (
                            <button
                                ...
                                type="button"
                                className="btn btn-xs btn-outline rounded-full"
                                onClick={() => setAddData({ ...addData, ExpensesDesc: item })}
                            >
                                {item}
                            </button>
                        ))}
                    ...
                    <div className="w-full">
                        <input type="number"
                            value={addData.ExpensesAmount}
                            onChange={(e) => setAddData({ ...addData, ExpensesAmount: parseFloat(e.target.value) })}
                            placeholder="Amount"
                            className="input input-bordered w-full" />
                    </div>
                    <div className="modal-action">
                        <div className="w-full flex justify-between">
                            <button className="btn btn-success" onClick={handleSaveAdd}>Save</button>
